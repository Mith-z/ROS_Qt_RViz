#include "./include/qt_rviz_demo/UIPromoteClass/multiselect_combobox.h"
...
MultiSelectComboBox::MultiSelectComboBox(QWidget *parent) : QComboBox(parent) {
  ...
  // QListView{outline: none;} 去掉项虚线框
  // 设置正常状态和选中为状态item的背景色相同
  selectItemView->setStyleSheet(
      "QListView{outline: none;} QListView::item {background:#BDD7FD;} "
  ...
  connect(this, QOverload<int>::of(&QComboBox::activated), [=](int index) {
    ...
    if (!hasExist) {
      ...
      QStandardItem *item =
          new QStandardItem(QIcon("://images/false.png"), text);
...
void MultiSelectComboBox::addItem(const QIcon &icon, const QString &text,
                                  const QVariant &userData) {
...
void MultiSelectComboBox::uncheckAll() {
  for (int row = 0; row < popupModel->rowCount(); row++) {
    QStandardItem *item = popupModel->item(row);
    if (item->checkState() == Qt::Checked) {
...
bool MultiSelectComboBox::eventFilter(QObject *watched, QEvent *event) {
  ...
  if (frame && frame == watched) {
    ...
    if (event->type() == QEvent::MouseButtonPress) {
      ...
      QRect rect(frame->mapToGlobal(QPoint(0, 0)),
                 QSize(frame->width(), frame->height()));
      QRect rect1(selectItemView->viewport()->mapToGlobal(QPoint(0, 0)),
                  QSize(selectItemView->viewport()->width(),
                        selectItemView->viewport()->height()));
      ...
      if (!rect.contains(globalPos) && !rect1.contains(globalPos)) {
        ...
        selectItemViewPress(
            selectItemView->viewport()->mapFromGlobal(globalPos));
      }
    }
  }
...
void MultiSelectComboBox::selectItemViewPress(QPoint pos) {
  ...
  if (index.isValid()) {
    ...
    // x图标的区域
    QRect iconRect =
        QRect(rect.left(), rect.top(), iconSize.width(), iconSize.height());
    //鼠标在x图标的区域内
